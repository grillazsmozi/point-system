...
io.on("connection", (socket) => {
  ...
  socket.on("loginRequest", ({ name }) => {
    ...
    const approvalUrl = `http://sorverseny.onrender.com/admin/approve?id=${token}`;
    pending[token] = socket;
...
app.get("/admin/approve", (req, res) => {
  ...
  console.log("Received approval link for token:", token);
  console.log("Current pending tokens:", Object.keys(pending));
  if (token && pending[token]) {
    pending[token].emit("loginApproved");
    delete pending[token];
    res.send(`<h1>Bejelentkezés engedélyezve</h1><p>Token: ${token}</p>`);
  } else {
    res.status(404).send("<h1>Érvénytelen vagy lejárt azonosító</h1>");
  }
});
